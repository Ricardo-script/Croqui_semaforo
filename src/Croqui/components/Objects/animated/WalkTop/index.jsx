...
export default function WalkTop(props){
	...
	const insertConfigModalSettings = () => {
        props.setPropsSettings({ type: 'pedestre', title: 'Pedestre subindo', image: walkPauseTop, object: props.walkTop, setObject: props.setWalkTop});
                ...
                if(props.walkTop[index] !== ''){
                    return(
                        <ItemDraggable key={index} index={index} object='walkTop' top={item.top} left={item.left} setWalkTop={props.setWalkTop} walkTop={props.walkTop}>
                            <MoveWalkTop src={props.moveWalkTop === true ? walkTop : walkPauseTop} alt="" move={props.moveWalkTop} tamanho={item.tamanho} velocidade={item.velocidade} percurso={item.percurso}/>
